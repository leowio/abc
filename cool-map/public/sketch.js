...
function draw() {
  ...
  if (mapInit) {
    ...
    me.update();
    me.display();
    for (let id in otherPoints) {
      otherPoints[id].update();
      otherPoints[id].display();
    }
    // console.log(me)
  }
}
...
function updateMapContent() {
  ...
  for (let id in otherPoints) {
    ...
    let posOnCanvas = myMap.latLngToPixel(device.latitude, device.longitude);
    otherPoints[id].goalX = posOnCanvas.x;
    otherPoints[id].goalY = posOnCanvas.y;
    otherPoints[id].accuracy = device.accuracy || 0;
  }
}
...
function syncOtherPoints() {
  ...
  for (let id in otherDevices) {
    ...
    let existingPoint = otherPoints[id];
    if (existingPoint) {
      nextPoints[id] = existingPoint;
      continue;
    }

    let point = new MyPoint();
    point.col = color(170, 190, 240);
    nextPoints[id] = point;
  }
